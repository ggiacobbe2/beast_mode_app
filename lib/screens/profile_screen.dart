import 'dart:io';
import 'package:beast_mode/screens/login_screen.dart';
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void _onTabTapped(int index) {
    ...
    switch (index) {
      ...
      case 2: Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const WorkoutLogScreen())); break;
        ...
        children: [
          ...
          Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(data['title'] as String? ?? 'Entry', style: const TextStyle(fontWeight: FontWeight.bold)),
                const SizedBox(height: 4),
                Text(data['caption'] as String? ?? '', maxLines: 3, overflow: TextOverflow.ellipsis),
            ...
            children: [
              Row(
                children: [
                  ...
                  IconButton(icon: const Icon(Icons.edit), onPressed: () => _editProfile(data)),
                ],
              ),
              ...
              const Text("Workouts", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
              ...
              StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                ...
                builder: (context, workoutSnap) {
                  ...
                  if (docs.isEmpty) return const Text("No workouts yet.");
                  return Column(children: docs.map(_workoutItem).toList());
                },
              ),
              const SizedBox(height: 16),
              const Text("Journal", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
              const SizedBox(height: 8),
              StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                ...
                builder: (context, journalSnap) {
                  ...
                  if (docs.isEmpty) return const Text("No journal entries yet.");
                  return Column(children: docs.map(_journalItem).toList());
                },
              ),
            ],
          );
  ...
  Future<void> _changePhoto(String uid) async {
    ...
    try {
      final url = await StorageService().uploadProfileImage(uid: uid, file: File(picked.path));
